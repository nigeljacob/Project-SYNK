  ...
  ipcMain.on("sendStartTask", (event, Task) => {
    ...
    setInterval(() => {
      ...
      if (currentlyTrackingApplication.appName !== currentWindow.info.name) {
        ...
        let oldIndex = trackedApplications.findIndex((app) => app.appName === currentWindow.info.name);
        trackedApplications[oldIndex] = currentlyTrackingApplication;

        if (trackedApplications.some((app) => app.appName === currentWindow.info.name)) {
          ...
          currentlyTrackingApplication = {appName: currentWindow.info.name, startTime: new Date(), endTime: null}
