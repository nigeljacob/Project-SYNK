...
function FolderViewer({ url }) {
  ...
  useEffect(() => {
    if(downloaded) {
      sendNotification("Download Succssfull", "The zip file has been downloaded. Please navigate to the relavent directory to access the file", "success", "success", auth.currentUser.uid)
    }
  }, [])
  ...
  const renderTree = (nodes) => (
    <TreeItem key={nodes.id} nodeId={nodes.id} label={nodes.name} onClick={() => handleFileClick(nodes.content, nodes.name)}>
        ...
        {selectedFileContent ?  (
          <div className='tw-mx-[10px]'>
            <div className='tw-flex tw-flex-row tw-items-center tw-mb-[10px] tw-relative tw-w-full'>
              <IOIcons.IoIosArrowBack className='tw-mr-[10px] tw-w-[20px] tw-h-[20px] tw-cursor-pointer' onClick={event => {
                          handleFileClick('', '')
                      }}/>
              <h3 className='tw-text-[14px]'>{selectedFileName}</h3>
              <div className='tw-flex tw-items-center t-flex-1'>
                <h3 className='tw-ml-[100px] tw-text-[#e9e949] tw-text-[12px] tw-mr-[10px]'>⚠️ This is a preview. Download the file to view it properly.</h3>
                <FaDownload className="tw-cursor-pointer tw-mr-[10px] tw-w-[20px] tw-h-[20px]" onClick={event => {
            ...
            <div className='tw-bg-black tw-p-[10px] tw-text-white tw-rounded-[5px] tw-text-[12px] tw-select-text tw-font-mono heightMain4'>
              {selectedFileName.endsWith('.jpg') || selectedFileName.endsWith('.png') || selectedFileName.endsWith('.jpeg')  ? (
                <img src={`data:image/png;base64,${selectedFileContent}`} alt={selectedFileName} />
              ) : (
                selectedFileName.endsWith('.pdf') ? (
                  <div className='tw-w-full tw-h-full tw-flex tw-justify-center tw-items-center'>
                    <h3>PDF Viewer is not supported. Download the file to view it</h3>
                  </div>
                ) : (
