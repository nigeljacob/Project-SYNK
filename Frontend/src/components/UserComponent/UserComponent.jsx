...
const UserComponent = ({ UID, type, setTriger }) => {
          ...
          read_from_Database_onChange("Teams/" + UID, (receievedTeams) => {
            {
              if (UID != auth.currentUser.uid) {
                for (let i = 0; i < receievedTeams.length; i++) {
                  if (
                    myTemams.some(
                      (team) => team.teamCode === receievedTeams[i].teamCode
                    )
                  ) {
                    count++;
                  }
  ...
  return (
    <div
      className="tw-absolute tw-mt-[-198px] tw-ml-[-20px] tw-w-[200px] tw-h-[200px] tw-max-h-[200px] tw-max-w-[200px] tw-bg-[#0B0B0B] tw-rounded-[10px] tw-p-[10px] tw-overflow-y-scroll tw-z-[1000]"
            ...
            <div>
              <div className="tw-w-full tw-flex tw-items-center">
                ...
                <div className="tw-ml-[10px]">
                  ...
                  <p className="tw-text-[10px] tw-text-[#A7A7A7]">
                    {user.userStatus}
                  </p>
                </div>
              </div>
              ...
              {UID != auth.currentUser.uid ? (
                teams != null ? (
                  <div className="tw-mt-[5px] tw-ml-[5px]">
                    <h5 className="tw-text-[12px] tw-font-bold">
                      Other Teams:{" "}
                    </h5>
                    {teams != 1 ? (
                      <p className="tw-text-[10px] tw-text-[#A7A7A7]">
                        In {teams - 1} other teams with you
                      </p>
                    ) : (
                      <p className="tw-text-[10px] tw-text-[#A7A7A7]">
                        Not in any other teams with you
                      </p>
                    )}
                  </div>
                ) : null
                ...
                <div className="tw-mt-[5px] tw-ml-[5px]">
                  ...
                  {teams - 1 === 0 ? (
                    <p className="tw-text-[10px] tw-text-[#A7A7A7]">
                      Not in any other Teams on SYNK
                    </p>
                  ) : (
                    <p className="tw-text-[10px] tw-text-[#A7A7A7]">
                      In {teams - 1} other teams on SYNK
                    </p>
                  )}
                </div>
              ) : null}
