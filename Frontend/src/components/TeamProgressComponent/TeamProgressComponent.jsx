...
const TeamProgressComponent = ({ photo, member, tasks, currentTeam, elementTrigger }) => {
  ...
  if (member.status != "Active") {
    for (let i = 0; i < tasks.length; i++) {
      if (
        tasks[i].taskStatus === "Continue" ||
        tasks[i].taskStatus === "In Progress"
      ) {
        tempStartedTasks++;
  ...
  return (
    <div className="tw-mt-[30px] tw-min-w-[255px] tw-min-h-[255px] tw-h-[255px] tw-bg-[#272727] tw-mb-[40px] tw-rounded-[10px] tw-py-[20px] tw-px-[15px] tw-flex tw-flex-col tw-justify-between">
          ...
          {profilePic != "" ? (
            <div className="tw-w-[50px] tw-h-[50px] tw-rounded-[5px] tw-bg-[#0B0B0B]">
              <img
                src={profilePic}
                alt="profile picture"
                className="tw-w-[50px] tw-h-[50px] tw-object-cover tw-rounded-[5px]"
              />
            </div>
          ) : (
          ...
          {StartedTasks > 0 ? (
            <>
              ...
              {tasks.length == 1 ? (
                ...
                <p className="tw-text-[14px]">
                  {tasks.length - 1} other task to complete
                </p>
              ) : (
                <p className="tw-text-[14px]">
                  {tasks.length - 1} other tasks to complete
                </p>
              )}
            </>
          ) : (
            ...
            <div>
              ...
              <div className="tw-flex tw-gap-[8px]">
                <img src={timeClock} alt="time clock" />
                {tasks.length == 0 ? (
                  <p className="tw-text-[14px]">No Tasks assigned yet</p>
                ) : tasks.length < 2 ? (
                  <p className="tw-text-[14px]">
                    {tasks.length} task to complete
                  </p>
                ) : (
                  <p className="tw-text-[14px]">
