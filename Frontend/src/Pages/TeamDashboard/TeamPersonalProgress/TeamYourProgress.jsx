...
const TeamYourProgress = (props) => {
  ...
  if(StartedTasks.length > 0) {
    ...
    const data = {
      labels: StartedTasks[progressIndex].progress.applicationTimeList.map((data) => data.name + " - " + formatMilliseconds(data.timeLength)),
      datasets: [
          {
              label: "Average Spent in percentage (%)",
              data: StartedTasks[progressIndex].progress.applicationTimeList.map((data) => Math.round((data.timeLength / StartedTasks[progressIndex].progress.taskLength) * 100)),
          },
  ...
  const handleVersionHistoryClick = () => {
    props.elementTrigger(<ProgressVersionHistory setElement={props.elementTrigger} team = {props.team} sideBarStatus={props.sideBarStatus} index = {progressIndex} UID = {props.UID}/>)
  }
...
{       
        ...
        <div className=" tw-flex tw-rounded-lg tw-py-10 tw-bg-[#272727] tw-h-[360px] tw-relative">
         ...
         <div className="tw-absolute tw-bottom-0 tw-right-0">
            <button className="tw-text-cyan-500 tw-bg-black tw-rounded-lg tw-p-1 tw-h-[35px] tw-w-[150px] tw-m-5" onClick={event => {handleVersionHistoryClick()}}>Version History</button>
         </div>
       </div>
       :
